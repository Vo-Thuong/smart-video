...
// ─── Left Panel ───────────────────────────────────────────────────────────────
function LeftPanel({
  transcript, loading, error, activeIndex, onSeek, onReview, completedSegments,
}: {
  transcript: TranscriptItem[]; loading: boolean; error: string | null;
  activeIndex: number; onSeek: (s: number) => void; onReview: (i: number) => void;
  ...
  useEffect(() => {
    ...
    const iTop = item.offsetTop, iBot = iTop + item.offsetHeight;
    if (iTop < top || iBot > bottom) {
      container.scrollTo({ top: iTop - container.clientHeight / 2 + item.offsetHeight / 2, behavior: "smooth" });
    }
  }, [activeIndex]);
        ...
        {loading ? (
          ...
          <>
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-xs text-[#00E5FF] font-medium">{checkedCount}/{transcript.length}</span>
            </div>
              ...
              return (
                <div
                  ...
                  className={`flex gap-3 rounded-xl p-3 cursor-pointer transition-all select-none border ${
                    isActive ? "bg-[#00E5FF]/15 border-[#00E5FF] shadow-[0_0_12px_rgba(0,229,255,0.2)]"
                      : isChecked ? "bg-[#00E5FF]/10 border-[#00E5FF]/30"
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 space-y-0.5">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-mono font-bold ${isActive ? "text-[#00E5FF]" : "text-gray-500"}`}>#{i + 1}</span>
                      <span className={`text-xs font-mono font-semibold px-2 py-0.5 rounded-full ${
                        isActive ? "bg-[#00E5FF] text-black" : isChecked ? "bg-[#00E5FF]/20 text-[#00E5FF]" : "bg-white/10 text-gray-400"
                      }`}>{line.time}</span>
                      {isActive && <span className="text-[10px] text-[#00E5FF] font-bold animate-pulse">▶</span>}
                    </div>
                    <p className={`text-sm leading-relaxed transition-colors ${
                      isActive ? "text-white font-medium" : isChecked ? "text-gray-400 line-through" : "text-gray-200"
                    }`}>{isComplete ? line.text : line.text.replace(/\S/g, "*")}</p>
                  </div>
                </div>
              );
            })}
...
function DictationPanel({
  videoTitle, studyIndex, transcript, isPlaying, onPlayPause, onReplay, onSkip,
  onSegmentComplete, completedSegments,
}: {
  videoTitle: string; studyIndex: number; transcript: TranscriptItem[];
  isPlaying: boolean; onPlayPause: () => void; onReplay: () => void; onSkip: () => void;
  onSegmentComplete: (i: number) => void; completedSegments: Set<number>;
}) {
  ...
  const [wordDetail, setWordDetail] = useState<{
    ...
    translation: string | null; loading: boolean; error: string | null;
  } | null>(null);
  const [vocabDialog, setVocabDialog] = useState<{
    word: string; phonetic: string; translation: string; example: string; note: string;
    videoTitle: string; segmentTime: string; saving: boolean; saved: boolean;
  } | null>(null);
  ...
  useEffect(() => {
    setInput(""); revealedRef.current = new Set(); setRevealedWords(new Set());
    setTranslation(null); setTranslationLoading(false); setIsCompleted(false);
  ...
  useEffect(() => {
    ...
    revealedRef.current = all; setRevealedWords(all); setIsCompleted(true);
    setTranslationLoading(true); setTranslation(null);
    fetch(`https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=vi&dt=t&q=${encodeURIComponent(seg.text)}`)
      .then((r) => r.json())
      .then((data) => setTranslation((data[0] as Array<[string]>).map((item) => item[0]).join("")))
  ...
  const handleInputChange = (value: string) => {
    ...
    const next = new Set<number>(); let cursor = 0;
    for (const token of tokens) {
      for (let j = cursor; j < words.length; j++) {
        if (normalizeWord(words[j]) === token) { next.add(j); cursor = j + 1; break; }
      }
    }
    revealedRef.current = next; setRevealedWords(new Set(next));
    if (next.size === words.length && words.length > 0) onSegmentComplete(studyIndex);
  };
  ...
  const handleWordClick = (word: string) => {
    ...
    fetch(`http://localhost:5000/api/dictionary/lookup?word=${encodeURIComponent(normalized)}`)
      .then((r) => r.json())
      .then((data) => {
        if (!data.success) { setWordDetail((p) => p && { ...p, loading: false, error: "Lỗi kết nối server" }); return; }
        setWordDetail({ word: normalized, phonetic: data.phonetic ?? null, dictMeanings: data.dictMeanings ?? [], translation: data.translation ?? null, loading: false, error: null });
      })
      .catch(() => setWordDetail((p) => p && { ...p, loading: false, error: "Lỗi kết nối server" }));
  };
  ...
  const openVocabDialog = () => {
    ...
    setVocabDialog({ word: wordDetail.word, phonetic: wordDetail.phonetic ?? "", translation: trans, example: seg?.text ?? "", note: "", videoTitle, segmentTime: seg?.time ?? "", saving: false, saved: false });
  };
  ...
  const saveVocab = async () => {
    ...
    try {
      ...
      await fetch("http://localhost:5000/api/vocabulary", {
        ...
        body: JSON.stringify({ word: vocabDialog.word, phonetic: vocabDialog.phonetic, translation: vocabDialog.translation, example: vocabDialog.example, note: vocabDialog.note, videoTitle: vocabDialog.videoTitle, segmentTime: vocabDialog.segmentTime }),
      });
    ...
    <>
      <div className="flex flex-col h-full">
        <div className="px-4 py-3 border-b border-white/10 flex-shrink-0 flex items-center gap-2">
          ...
          {transcript.length > 0 && (
            <span className="ml-auto text-xs text-gray-500">{Math.max(0, studyIndex + 1)}/{transcript.length}</span>
          )}
        </div>
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5">
          ...
          <div className="flex items-center justify-center gap-3 pt-1">
            {[
              { label: "Replay", icon: <RotateCcw className="w-4 h-4" />, action: onReplay },
              { label: isPlaying ? "Pause" : "Play", icon: isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />, action: onPlayPause },
              { label: "Skip", icon: <SkipForward className="w-4 h-4" />, action: onSkip },
            ].map(({ label, icon, action }) => (
              <button key={label} onClick={action} className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10 hover:text-white transition-all text-sm">
                {icon} {label}
              </button>
            ))}
          </div>
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <textarea
              ...
              onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); onSkip(); } }}
              rows={3} placeholder="Gõ ở đây..."
          ...
          <button onClick={onSkip} className="w-full py-2.5 rounded-xl bg-[#00E5FF] text-black font-semibold text-sm hover:bg-[#00BCCC] active:scale-95 transition-all flex items-center justify-center gap-2">
                ...
                {words.map((word, i) => (
                  <button key={i} onClick={() => handleWordClick(word)}
                    className={`px-2.5 py-1 rounded-lg text-sm border leading-none transition-all duration-300 cursor-pointer hover:opacity-80 ${
                      revealedWords.has(i) ? "bg-[#00E5FF]/10 border-[#00E5FF]/40 text-white font-medium" : "bg-[#2D1F47] border-white/10 text-gray-200 font-mono tracking-widest"
                    }`}>
          ...
          {wordDetail && (
            <div className="rounded-xl border border-white/15 bg-[#2D1F47] p-3 flex flex-col gap-2 relative">
              <button onClick={() => setWordDetail(null)} className="absolute top-2 right-2 text-gray-500 hover:text-white transition-colors">
                <X className="w-3.5 h-3.5" />
              </button>
              <div className="flex items-center gap-2 pr-5">
                <span className="text-base font-bold text-white">{wordDetail.word}</span>
                {wordDetail.phonetic && <span className="text-xs text-gray-400">{wordDetail.phonetic}</span>}
                <button onClick={() => { window.speechSynthesis.cancel(); const u = new SpeechSynthesisUtterance(wordDetail.word); u.lang = "en-US"; u.rate = 0.8; window.speechSynthesis.speak(u); }} className="ml-auto text-[#00E5FF] hover:opacity-80 transition-opacity flex-shrink-0" title="Phát âm">
                  <Volume2 className="w-3.5 h-3.5" />
                </button>
              </div>
              {wordDetail.loading ? (
                <div className="flex items-center gap-1.5 text-gray-500 text-xs"><Loader2 className="w-3 h-3 animate-spin" /> Đang tải...</div>
              ) : wordDetail.error ? (
              ...
              {!wordDetail.loading && (
                <button onClick={openVocabDialog} className="mt-1 flex items-center gap-1.5 text-xs text-[#7C3AED] hover:text-[#9D5CF6] transition-colors font-medium">
          ...
          {isCompleted && (
            <div className="flex flex-col gap-1.5 border-t border-white/10 pt-3">
              ...
              {translationLoading ? (
                <div className="flex items-center gap-1.5 text-gray-500 text-xs"><Loader2 className="w-3 h-3 animate-spin flex-shrink-0" /> Đang dịch...</div>
              ) : translation ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-[#1C1132] border border-white/15 rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="p-5 flex flex-col gap-3">
              {([
                ...
                <div key={key} className="flex flex-col gap-1">
                  ...
                  <input value={vocabDialog[key]} onChange={(e) => setVocabDialog((v) => v && { ...v, [key]: e.target.value })} placeholder={placeholder}
            ...
            <div className="px-5 pb-5 flex gap-2">
              <button onClick={() => setVocabDialog(null)} className="flex-1 py-2.5 rounded-xl border border-white/15 text-gray-300 hover:bg-white/5 text-sm transition-colors">Cancel</button>
              <button onClick={saveVocab} disabled={vocabDialog.saving || vocabDialog.saved}
                ...
                {vocabDialog.saving ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Saving...</> : vocabDialog.saved ? "✓ Saved!" : "Save"}
              </button>
            </div>
          </div>
        </div>,
...
export default function LocalPracticePage() {
  ...
  const [videoInfo, setVideoInfo] = useState<{ title: string; videoUrl: string } | null>(null);
  ...
  useEffect(() => {
    ...
    const token = localStorage.getItem("token");
    fetch(`http://localhost:5000/api/video/local/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.success) {
          ...
          if (token) {
            fetch(`http://localhost:5000/api/saved-video/${id}/local-practice`, {
              method: "POST",
  ...
  const saveProgress = useCallback((segIdx: number) => {
    const video = videoRef.current;
    const token = localStorage.getItem("token");
    ...
    fetch(`http://localhost:5000/api/saved-video/${id}/local-progress`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({ progressTime: time, progressSegment: segText, duration: dur }),
    }).catch(() => {});
  }, [id, transcript]);
  ...
  return (
    <div className="flex h-full bg-gradient-to-br from-[#3E2465] to-[#1C1642]">
      ...
      <aside className="w-72 flex-shrink-0 border-r border-white/10 flex flex-col bg-[#1C1132]/60 backdrop-blur-sm">
        ...
        <LeftPanel
          transcript={transcript} loading={transcriptLoading} error={transcriptError}
          activeIndex={activeIndex} onSeek={handleSeek} onReview={handleReview}
      ...
      <aside className="w-80 flex-shrink-0 border-l border-white/10 flex flex-col bg-[#1C1132]/60 backdrop-blur-sm">
        <DictationPanel
          videoTitle={title} studyIndex={studyIndex} transcript={transcript}
          isPlaying={isPlaying} onPlayPause={handlePlayPause} onReplay={handleReplay}
          onSkip={handleSkip} onSegmentComplete={handleSegmentComplete}
          completedSegments={completedSegments}
        />
      </aside>
    </div>
