...
export default function NotificationsPage() {
  ...
  const [filter, setFilter] = useState<"all" | "unread" | "liked" | "friends-posts">("all");
      ...
      try {
        ...
        if (friendsRes.ok) {
          ...
          if (data.success) {
            ...
            localStorage.setItem("feed_friends_last_seen", Date.now().toString());
          }
        }
      } catch {
        // silent
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTimeout(() => {
          ...
          setStatuses((prev) => { const s = { ...prev }; delete s[requestId]; return s; });
        }, 1200);
      }
    } catch {
  ...
  const lastSeen = parseInt(typeof window !== "undefined" ? localStorage.getItem("notifications_last_seen") || "0" : "0", 10);

  const filteredRequests =
    filter === "unread"
      ? requests.filter((r) => !readIds.has(r._id) && new Date(r.createdAt).getTime() > lastSeen - 86400000 * 30)
      : requests;
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white/90 leading-snug">
                      <span className="font-semibold text-white">{req.sender.fullname}</span>
                      {" "}
                      <span className="text-white/60">{n.friendRequest}</span>
                    </p>
                    <p className="text-xs text-white/30 mt-0.5">{timeAgo(req.createdAt, lang)}</p>
                  </div>
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    {status === "accepted" ? (
                      ...
                      <>
                        ...
                        <button
                          onClick={(e) => { e.stopPropagation(); handleDecline(req._id); }}
                          disabled={status === "accepting" || status === "declining"}
            ...
            <div className="flex flex-col gap-2">
              {likedPosts.map((post) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-white/85 truncate">{post.title || post.caption || "—"}</p>
                    <p className="text-xs text-white/30 mt-0.5">{timeAgo(post.createdAt, lang)}</p>
                  </div>
                ...
                >
                  ...
                  <div className="relative flex-shrink-0">
                    {post.userId?.avatar ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={post.userId.avatar.startsWith("http") ? post.userId.avatar : `http://localhost:5000${post.userId.avatar}`}
                        alt=""
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-white/90">
                      <span className="font-semibold text-white">{post.userId?.fullname || post.userId?.username}</span>
                      <span className="text-white/50"> {n.activityPosted}</span>
                    </p>
                    <p className="text-xs text-white/55 truncate mt-0.5">{post.title || post.caption || "—"}</p>
                    <p className="text-xs text-white/30 mt-0.5">{timeAgo(post.createdAt, lang)}</p>
                  </div>
                  {/* Thumbnail */}
                  {post.thumbnail && (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={post.thumbnail} alt="" className="w-14 h-10 object-cover rounded-lg flex-shrink-0 opacity-70" />
                  )}
                </Link>
              ))}
            </div>
