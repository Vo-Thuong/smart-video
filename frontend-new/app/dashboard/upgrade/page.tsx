...
import { Check, Zap, Infinity, BookOpen, Sparkles, Crown, ShieldCheck, RefreshCw, ArrowDownLeft, X, AlertTriangle } from "lucide-react";
...
function DowngradeModal({ onConfirm, onClose }: { onConfirm: () => void; onClose: () => void }) {
  const { t } = useLang();
  const p = t.upgrade;
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="bg-gradient-to-b from-[#2a1845] to-[#1e1235] border border-white/10 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
        ...
        <h3 className="text-white font-bold text-base mb-2">{p.downgradeModal.title}</h3>
        <p className="text-white/50 text-sm mb-1">
          {p.downgradeModal.body}
        </p>
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-white/60 text-sm">
              {p.activeBanner.plan.replace("{label}", active.label).replace("{price}", formatPrice(active.price)).replace("{unit}", active.unit)}
            </p>
            <p className="text-white/40 text-xs mt-1">
              {p.activeBanner.activated.replace("{date}", formatDate(active.activatedAt))}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-1.5 sm:items-end">
            {p.proFeaturesList.slice(0, 3).map((f) => (
              <span key={f} className="flex items-center gap-1.5 text-white/50 text-xs">
...
export default function UpgradePage() {
  ...
  const features = [
    { icon: Infinity, title: p.features[0].title, description: p.features[0].desc },
    { icon: BookOpen, title: p.features[1].title, description: p.features[1].desc },
    { icon: Sparkles, title: p.features[2].title, description: p.features[2].desc },
  ];

  const plans = [
    { id: "weekly",  label: p.plans.weekly,  price: "49.000",  rawPrice: 49000,  unit: p.plans.units.week,  badge: null,          saving: null },
    { id: "monthly", label: p.plans.monthly, price: "149.000", rawPrice: 149000, unit: p.plans.units.month, badge: p.plans.popular, saving: p.plans.save25 },
    { id: "yearly",  label: p.plans.yearly,  price: "999.000", rawPrice: 999000, unit: p.plans.units.year,  badge: p.plans.best,    saving: p.plans.save57 },
  ];
  ...
  useEffect(() => {
    ...
    if (token) {
      fetch("http://localhost:5000/api/auth/me", {
        ...
        .then((r) => r.json())
        .then((data) => {
          if (data.success && data.user.is_premium && data.user.premiumPlan?.planId) {
        ...
        .catch(() => {
          ...
          if (raw) {
            try { setActivePlan(JSON.parse(raw)); } catch {}
          }
        });
    }
  }, []);
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#1a0f2e] via-[#2a1845] to-[#1e1235] px-4 py-10 flex flex-col items-center">
      ...
      <div className="text-center mb-10 max-w-xl">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-white leading-tight">
          {isProActive ? (
            <>{p.onPro.replace("SmartVideo", "")}{" "}<span className="text-transparent bg-clip-text bg-gradient-to-r from-[#a78bfa] to-[#818cf8]">SmartVideo Pro</span></>
          ) : (
            <>{p.unlock.replace("SmartVideo", "")}{" "}<span className="text-transparent bg-clip-text bg-gradient-to-r from-[#a78bfa] to-[#818cf8]">SmartVideo</span></>
          )}
        </h1>
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-white/60 text-xs font-medium uppercase tracking-wider">{plan.label}</span>
                    {isCurrentPlan && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 border border-emerald-500/30">
                      {p.currentPlan}
                      </span>
                  ...
                  <div className="mb-1">
                    ...
                    <span className="text-white/40 text-sm ml-1">/ {plan.unit}</span>
                  </div>
                  {plan.saving
                    ? <span className="text-emerald-400 text-xs font-medium">{plan.saving}</span>
          ...
          {p.proFeaturesList.map((item) => (
            <li key={item} className="flex items-center gap-2.5 text-white/60 text-sm">
              <span className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 ${
                ...
                <Check className={`w-2.5 h-2.5 ${isProActive ? "text-emerald-400" : "text-[#c4b5fd]"}`} strokeWidth={3} />
              </span>
              {item}
            </li>
          ))}
