...
// ─── Plan definitions ─────────────────────────────────────────────────────────
const PLANS: Record<PlanId, { label: string; price: number; unit: string; saving: string | null; badge: string | null }> = {
  weekly:  { label: "Hàng tuần",  price: 49000,  unit: "tuần",  saving: null,            badge: null },
  monthly: { label: "Hàng tháng", price: 149000, unit: "tháng", saving: "Tiết kiệm 25%", badge: "Phổ biến nhất" },
  yearly:  { label: "Hàng năm",   price: 999000, unit: "năm",   saving: "Tiết kiệm 57%", badge: "Tốt nhất" },
};
...
const BANKS = [
  ...
  { code: "AGR",  name: "Agribank",                  fullName: "Ngân hàng Nông nghiệp & PTNT Việt Nam" },
  ...
  { code: "CBB",  name: "CB Bank",                   fullName: "Ngân hàng TMCP Xây dựng Việt Nam" },
  { code: "EIB",  name: "Eximbank",                  fullName: "Ngân hàng TMCP Xuất Nhập Khẩu VN" },
  ...
  { code: "PGB",  name: "PG Bank",                   fullName: "Ngân hàng TMCP Xăng dầu Petrolimex" },
  { code: "SCB",  name: "Saigon Bank",               fullName: "Ngân hàng TMCP Sài Gòn Công Thương" },
  ...
  { code: "VCCB", name: "VietCapital Bank",          fullName: "Ngân hàng TMCP Bản Việt" },
  { code: "IVB",  name: "Indovina Bank",             fullName: "Ngân hàng TNHH Indovina" },
  { code: "WOO",  name: "Woori Bank Việt Nam",       fullName: "Ngân hàng Woori Việt Nam" },
  { code: "HSBC", name: "HSBC Việt Nam",             fullName: "Ngân hàng TNHH MTV HSBC (Việt Nam)" },
  { code: "SCB2", name: "Standard Chartered VN",     fullName: "Ngân hàng TNHH MTV Standard Chartered VN" },
  { code: "CITI", name: "Citibank Việt Nam",         fullName: "Chi nhánh Citibank N.A. tại Việt Nam" },
  { code: "UOB",  name: "UOB Việt Nam",              fullName: "Ngân hàng United Overseas Bank VN" },
  { code: "SHIN", name: "Shinhan Bank Việt Nam",     fullName: "Ngân hàng TNHH MTV Shinhan Việt Nam" },
  { code: "PUB",  name: "Public Bank Việt Nam",      fullName: "Ngân hàng TNHH MTV Public Việt Nam" },
];
...
// ─── Bank Selector ────────────────────────────────────────────────────────────
function BankSelector({ value, onChange }: { value: string; onChange: (code: string) => void }) {
      ...
      >
        {selected ? (
          <span className="font-medium">{selected.name}{" "}
            <span className="text-white/50 font-normal text-xs">— {selected.fullName}</span>
          </span>
        ) : (
          <span className="text-white/40">Chọn ngân hàng...</span>
        )}
        <ChevronDown className={`w-4 h-4 text-white/40 transition-transform flex-shrink-0 ml-2 ${open ? "rotate-180" : ""}`} />
      </button>
      ...
      {open && (
        <div className="absolute z-50 mt-2 w-full rounded-xl bg-[#1e1235] border border-white/15 shadow-2xl overflow-hidden">
          ...
          <ul className="max-h-52 overflow-y-auto divide-y divide-white/5">
            {filtered.length === 0 && (
              <li className="px-4 py-3 text-white/40 text-sm">Không tìm thấy ngân hàng</li>
            )}
            {filtered.map((bank) => (
              <li key={bank.code}>
                <button
                ...
                >
                  ...
                  {value === bank.code && <Check className="w-3.5 h-3.5 text-[#a78bfa] ml-auto flex-shrink-0" />}
                </button>
              </li>
            ))}
          </ul>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0e0720]/90 backdrop-blur-sm px-4">
      <div className="bg-gradient-to-b from-[#2a1845] to-[#1e1235] border border-white/10 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl">
        ...
        <h2 className="text-white font-bold text-xl mb-2">Thanh toán thành công!</h2>
        <p className="text-white/50 text-sm mb-1">
          Gói <span className="text-white font-medium">{plan.label}</span> đã được kích hoạt.
        </p>
        <p className="text-white/40 text-xs mb-7">Cảm ơn bạn đã tin tưởng SmartVideo Pro ✨</p>
        <button
...
const inputCls = (hasError: boolean) =>
  `w-full px-4 py-3 rounded-xl bg-white/5 border text-white text-sm placeholder-white/30 outline-none transition-colors ${
    hasError ? "border-red-500/60 focus:border-red-400" : "border-white/10 focus:border-[#a78bfa]/60"
  }`;
...
function InfoRow({ label, value, copyable, highlight }: { label: string; value: string; copyable?: boolean; highlight?: boolean }) {
  ...
  return (
    <div className="flex items-center justify-between gap-2">
      ...
      <span className={`font-medium text-xs text-right ${highlight ? "text-[#c4b5fd]" : "text-white"}`}>{value}</span>
      {copyable && (
...
function CheckoutContent() {
  ...
  function validate() {
    ...
    if (method === "card") {
      ...
      if (cardNumber.replace(/\s/g, "").length < 16) e.cardNumber = "Số thẻ không hợp lệ";
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (token) {
        await fetch("http://localhost:5000/api/auth/upgrade", {
          ...
          body: JSON.stringify({ planId, label: plan.label, price: plan.price, unit: plan.unit }),
        });
      }
    } catch {
    ...
    // Cập nhật cache plan
    localStorage.setItem("smartvideo_pro_plan", JSON.stringify({
      planId, label: plan.label, price: plan.price, unit: plan.unit,
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0e0720] via-[#1a0f2e] to-[#0e0720] text-white">
      ...
      <div className="border-b border-white/8 px-6 py-4 flex items-center">
        ...
        <div className="mx-auto flex items-center gap-2">
          <span className="font-bold text-white text-base">Smart<span className="text-[#a78bfa]">Video</span></span>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 items-start">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <section className="bg-white/4 border border-white/8 rounded-2xl p-5">
            <h2 className="text-white font-semibold text-sm mb-4">Chu kỳ thanh toán</h2>
            <div className="grid grid-cols-3 gap-3">
              {(Object.entries(PLANS) as [PlanId, typeof PLANS[PlanId]][]).map(([id, p]) => (
                ...
                >
                  <div className="text-white/60 text-[10px] font-medium uppercase tracking-wider mb-1">{p.label}</div>
                  <div className="text-white font-bold text-base">{formatPrice(p.price)}₫</div>
                  {p.saving && <div className="text-emerald-400 text-[10px] font-medium mt-0.5">{p.saving}</div>}
                  {planId === id && (
          ...
          {/* Payment method */}
          <section className="bg-white/4 border border-white/8 rounded-2xl p-5">
            <h2 className="text-white font-semibold text-sm mb-4">Phương thức thanh toán</h2>
            <div className="grid grid-cols-2 gap-3 mb-6">
              {([
                { id: "card" as const,  label: "Thẻ tín dụng / ghi nợ",    icon: CreditCard },
                { id: "bank" as const,  label: "Chuyển khoản ngân hàng",    icon: Landmark },
              ] as { id: PaymentMethod; label: string; icon: React.ElementType }[]).map(({ id, label, icon: Icon }) => (
                <button key={id} type="button" onClick={() => { setMethod(id); setErrors({}); }}
              ...
              <div className="space-y-4">
                <Field label="Tên chủ thẻ" error={errors.cardName}>
                  <input value={cardName} onChange={(e) => setCardName(e.target.value)}
                    placeholder="NGUYEN VAN A" className={inputCls(!!errors.cardName)} />
                </Field>
                <Field label="Số thẻ" error={errors.cardNumber}>
                  <div className="relative">
                    <input value={cardNumber} onChange={(e) => setCardNumber(formatCardNumber(e.target.value))}
                      placeholder="0000 0000 0000 0000" className={inputCls(!!errors.cardNumber) + " pr-12"} />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <Field label="Ngày hết hạn" error={errors.expiry}>
                    <input value={expiry} onChange={(e) => setExpiry(formatExpiry(e.target.value))}
                      placeholder="MM/YY" className={inputCls(!!errors.expiry)} />
                  </Field>
                  <Field label="CVV / CVC" error={errors.cvv}>
                    <input value={cvv} onChange={(e) => setCvv(e.target.value.replace(/\D/g, "").slice(0, 4))}
                      placeholder="•••" type="password" className={inputCls(!!errors.cvv)} />
                  </Field>
                </div>
                <div className="flex items-center gap-2 pt-1">
                  {["VISA", "MC", "JCB", "AMEX", "NAPAS"].map((c) => (
                    <span key={c} className="px-2 py-1 rounded bg-white/8 border border-white/10 text-[10px] font-bold text-white/60 tracking-wider">
                      {c}
                    </span>
                  ))}
                </div>
              ...
              <div className="space-y-5">
                ...
                {selectedBank && (
                  <div className="bg-[#a78bfa]/8 border border-[#a78bfa]/20 rounded-xl p-4 space-y-3 text-sm">
                    ...
                    <InfoRow label="Ngân hàng" value={BANKS.find((b) => b.code === selectedBank)?.name ?? ""} />
                    <InfoRow label="Số tài khoản" value="1900 1234 5678 90" copyable />
                    <InfoRow label="Chủ tài khoản" value="CONG TY TNHH SMARTVIDEO" />
                    <InfoRow label="Số tiền" value={`${formatPrice(plan.price)}₫`} highlight />
                    <InfoRow
                      label="Nội dung CK"
                    ...
                    <p className="text-white/40 text-xs pt-2 border-t border-white/8">
                      Vui lòng chuyển khoản đúng số tiền và nội dung. Gói Pro kích hoạt trong 5–15 phút sau khi nhận thanh toán.
                    </p>
                  </div>
                )}
              </div>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? (
              ...
              <><Lock className="w-4 h-4" />
                {method === "card" ? `Thanh toán ${formatPrice(plan.price)}₫` : "Tôi đã chuyển khoản"}
              </>
            )}
          </button>
          ...
          <div className="flex flex-wrap items-center justify-center gap-5 text-white/30 text-xs">
            <span className="flex items-center gap-1.5"><ShieldCheck className="w-3.5 h-3.5" /> Bảo mật SSL 256-bit</span>
            <span className="flex items-center gap-1.5"><Lock className="w-3.5 h-3.5" /> Dữ liệu thẻ được mã hoá</span>
            <span className="flex items-center gap-1.5"><Check className="w-3.5 h-3.5" /> Huỷ bất kỳ lúc nào</span>
          </div>
        </form>
        ...
        <aside className="space-y-4 lg:sticky lg:top-8">
          ...
          <div className="bg-gradient-to-b from-[#2a1845]/80 to-[#1e1235]/80 border border-white/10 rounded-2xl p-5 backdrop-blur-sm">
            <div className="flex items-start justify-between mb-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-white font-bold text-base">SmartVideo Pro</span>
                </div>
            ...
            <div className="space-y-2 text-sm border-t border-white/8 pt-4">
              ...
              <div className="flex justify-between text-white font-bold text-base">
                <span>Tổng cộng <span className="text-white/40 font-normal text-xs">(đã gồm thuế)</span></span>
          ...
          <div className="bg-white/4 border border-white/8 rounded-2xl p-5">
            <h3 className="text-white/70 text-xs font-semibold uppercase tracking-wide mb-3">Quyền lợi bao gồm</h3>
            <ul className="space-y-2.5">
              {[
                ...
                <li key={text} className="flex items-center gap-2.5 text-white/60 text-sm">
                  <span className="w-4 h-4 rounded-full bg-[#a78bfa]/15 flex items-center justify-center flex-shrink-0">
                    <Icon className="w-2.5 h-2.5 text-[#c4b5fd]" strokeWidth={3} />
                  </span>
                  {text}
                </li>
              ))}
          ...
          <div className="flex items-start gap-3 bg-emerald-500/8 border border-emerald-500/20 rounded-2xl p-4">
            ...
            <div>
              <p className="text-emerald-300 font-semibold text-sm">Hoàn tiền trong 7 ngày</p>
